...
class PhDWorker(Worker):
    ...
    def get_google_scholar_info(self, dbname):
        ...
        len = db.get_number_of_all_phds()

        print len
        id = 1

        for i in range(0, len):
            phd = db.select_by_id(id)
            hindex, citations = google_scraper.get_hindex_and_citations(phd.firstname, phd.lastname)
            db.update_google_scholar(phd, citations, hindex)
            phd = db.select_by_id(id)
            print phd.to_string()
            id += 1
            time.sleep(200)
    ...
    def get_dblp_num_of_all_pubs_info(self, dbname):
        ...
        for i in range(0, len):
            ...
            db.update_dblp_num_of_all_pubs(phd, num_of_pubs)
            phd = db.select_by_id(id)
            print phd.to_string()
            id += 1
            time.sleep(200)
    ...
    def get_scopus_num_of_all_pubs_info(self, dbname, keyword):
        ...
        for i in range(0, len):
            ...
            num_of_pubs = scopus.get_number_of_publications(phd.firstname, phd.lastname, keyword)
            hindex = scopus.get_hindex(phd.firstname, phd.lastname, keyword)

            db.update_scopus(phd, num_of_pubs, hindex)
            phd = db.select_by_id(id)
...
if __name__ == "__main__":
    ...
    w.get_scopus_num_of_all_pubs_info('POLon_PhD_IT_DB.sql', 'comp')

    # w.get_dblp_num_of_all_pubs_info('POLon_PhD_IT_DB.sql')
    # w.get_scopus_num_of_all_pubs_info('POLon_PhD_IT_DB.sql', 'comp')

    # w.get_google_scholar_info('POLon_PhD_IT_UMCS_DB.sql')
    # w.get_dblp_num_of_all_pubs_info('POLon_PhD_IT_UMCS_DB.sql')
    # w.get_scopus_num_of_all_pubs_info('POLon_PhD_IT_UMCS_DB.sql', 'comp')

    # w.get_google_scholar_info('POLon_PhD_MATH_UMCS_DB.sql')
    # w.get_dblp_num_of_all_pubs_info('POLon_PhD_MATH_UMCS_DB.sql')
    # w.get_scopus_num_of_all_pubs_info('POLon_PhD_MATH_UMCS_DB.sql', 'math')
